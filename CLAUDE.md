...
The **projects section** is a grid of six `.project-card`s. Each card leads with its title, then a `.project-stats` highlight row — big serif numbers with directional color (`.pstat-num.down` red for an overvaluation call, `.pstat-num.up` green for the live dashboard) — then a description and `.project-tag`s. Demo-backed cards (3M, Costco, mortgage, stocks) end with a `.project-actions` row: a primary "Live Demo ↗" to the leaf page plus an outline "Download …" to the source file in `assets/`. Description-only cards (Financial Metrics Dashboard, FP&A SQL) carry no buttons. There are **no emojis or "Live Demo" badges** — both were removed, so the button alone signals the demo; the leaf pages must not link to themselves (see the DCF section). The `.project-icon` / `.project-badge` / `.project-arrow` CSS rules are now unused leftovers.
